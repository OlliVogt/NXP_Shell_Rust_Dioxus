...
fn read_and_print_data(port : &mut Box<dyn SerialPort>, cmd: &str) {
    ...
    // let mut port: Box<dyn SerialPort> = open_serial_port();
    println!("send: {}", cmd);
    ...
    // &str::from_utf8(&**in_buffer.get()).unwrap())
    let _n = match port.read(&mut input_buf[..]) {
    // let _n = match port.read( &mut in_buffer.get()[..]) {
        Ok(s) => s,
        Err(e) => {
    ...
    // str::from_utf8(&in_buffer).unwrap()
}